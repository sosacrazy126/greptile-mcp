...
def test_everything():
    ...
    # 4. Test tool execution without auth (should return error in result)
    ...
    assert response.status_code == 200  # MCP returns 200 with error in result
    data = response.json()
    result_text = data["result"][0]["text"]
    assert "Error" in result_text or "401" in result_text or "404" in result_text
    print("✓ Tool execution properly returns error without authentication")
